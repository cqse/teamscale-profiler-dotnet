...
/**
* Writes information about the profiled process (path to exe) to the output file.
*/
...
*/
HRESULT CProfilerCallback::GetFunctionIdentifier( FunctionID functionID, FunctionInfo* info) {
...
*/
void CProfilerCallback::WriteToLog(const char* label, vector<FunctionInfo>* list) {
	for (vector<FunctionInfo>::iterator i = list->begin (); i != list->end (); i++) {
		FunctionInfo info = *i;
